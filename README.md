...
The Soft-NMS repository also contains code for training these detectors on multiple GPUs. The position sensitive ROI Pooling is updated so that interpolation of bins is correct, like Mask-RCNN. The COCO detection model for R-FCN can be found [here](https://drive.google.com/open?id=0B6T5quL13CdHX04xN1ZQX2IyMms). All other detection models used in the paper are publicly available.
